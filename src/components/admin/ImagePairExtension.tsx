...
function ImagePairView({ node, updateAttributes, deleteNode, selected }: any) {
  ...
  const uploadFileToSide = async (file: File, side: "left" | "right") => {
  ...
  const handleUploadSecond = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    await uploadFileToSide(file, targetSideRef.current);
    if (e.target) e.target.value = "";
  };

  const handlePasteSlot = async (e: React.ClipboardEvent<HTMLDivElement>, side: "left" | "right") => {
    ...
    if (!file) return;
    await uploadFileToSide(file, side);
  };

  // handleUploadSecond replaced by above
      ...
      >
        ...
        {!isPaired && (
          <div
            ...
            onPaste={(e) => handlePasteSlot(e, "left")}
          >
            <div
        ...
        {isPaired ? (
          ...
          <div
            ...
            onPaste={(e) => handlePasteSlot(e, "right")}
          >
            <div
              className="image-pair-circle-btn"
